...
> 이 문서는 2026-07-18 기준의 QA 범위와 날짜별 실행 기록입니다. 현재 작업의 완료 기준은 [AGENTS.md의 작업 범위와 완료 기준](../AGENTS.md#작업-범위와-완료-기준)을 따릅니다. 아래 제외 항목이나 과거 PASS 기록은 현재 사용자가 요청한 검증을 면제하지 않습니다.
...
- 이 계획 전체를 검증하는 작업은 Q1~Q6 전부 통과해야 완료한다. 현재 요청에서 추가한 항목도 별도로 검증한다.
- Q5는 헤드리스 브라우저로 검증하되, 불가한 항목은 미검증으로 표시하고 사유와 후속 수동 체크리스트를 남긴다. 체크리스트 작성은 검증 통과가 아니며, 독립적으로 가능한 검증은 계속한다.
- 재실행 결과는 날짜를 구분한 새 "QA 결과" 섹션에 기록하고, 과거 실행 기록을 현재 결과로 덮어쓰지 않는다.

## 기본 계획의 범위 외

- 부하/성능 테스트, 크로스브라우저 매트릭스, 실기기 GPS 테스트(현지 필요)는 이 기본 계획에 포함하지 않는다. 단, 현재 사용자 요청에 포함되면 작업 범위에 추가한다. 필요한 장비·현지 접근 등이 없으면 해당 항목을 미완료로 남기고 필요한 조건을 보고한다.
