...
- 검증 표시: 근사치 / 출처 대조·현장 미검증 / 현장 GPS 검증을 분리. Google Maps 오류 배지와 독립적으로 표시.
- 모바일: 상세 → 목록 복귀, 검색·카테고리 변경 시 선택 해제, 시트 손잡이의 키보드 조작 및 터치 영역 개선.
- ISS-005 재수정: 고정 일본어 초기화·전역 i18n을 제거. `proxy.ts`에서 URL → 쿠키 → Accept-Language → 일본어 순으로 판별하고 SSR/클라이언트가 같은 언어 사용. 전환 시 MDX/RSC도 갱신.
- 아일랜드뷰: 공식 GTFS 승강장 ID로 12개 좌표 대체. 근사치 5곳 제거, 7/9번 방향 구분, 12번 이름 통일. 2026년 10월 개칭 예고는 별도 안내.
- 경로: 시티뷰·야경은 공식 노선도 `line.php`; 아일랜드뷰 A/B는 공식 PDF와 대조한 OSM 도로 참고 형상. 자동차 최단 경로가 우회하는 10→11 산길은 OSM way 연결로 보정.
- 시간표: 실제 정류장별 간격을 계산해 B코스의 60분 배차와 불규칙 배차를 정확히 표시. 야경 운행일은 일본 시간 기준.
- Next.js 16.3.5 / NextAuth beta.32 및 호환 의존성 보안 업데이트. 관리자 무인증 차단 회귀 유지.
- Vitest·Playwright·GitHub Actions 도입. 실행 결과와 한계는 `docs/qa/2026-09-20-reliability.md` 참조.

아래 항목은 당시의 원인·해결 기록이며, 현재 구현은 위 내용으로 대체된다.
...
- 부수 효과: 첫 방문 비기본 언어 사용자는 짧은 기본 언어 플래시 후 전환 (스토리 RSC 콘텐츠는
  쿠키가 생긴 다음 방문부터 감지 언어로 제공).

> **2026-07-18 변경:** 기본 언어가 ko → ja로 변경되어 동일 패턴으로 `lng: 'ja'` 고정 중.
