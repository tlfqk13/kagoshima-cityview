...
> 최종 업데이트: 2026-09-20 (신뢰성·모바일·다국어·노선 데이터·회귀 테스트 개선)

## 최신 상태 — 2026-09-20

- 현장 검증/출처 확인/근사 좌표 배지 분리, 모바일 목록 복귀·검색, 요청별 SSR 언어 일치 적용.
- 공식 GTFS 및 PDF 대조로 아일랜드뷰 12개 정류장 좌표·방향별 승강장·시간표를 교정. 새 현장 실측은 수행하지 않음.
- 시티뷰·야경은 공식 웹 노선 형상, 아일랜드뷰는 OSM 도로와 공식 PDF를 대조한 A/B 참고 경로 적용. 산길 우회 오류 교정.
- 운행일은 일본 표준시 기준. Next.js 16.3.5 / NextAuth beta.32, `src/proxy.ts` 사용.
- Vitest·Playwright·GitHub Actions 추가. 상세 검증 및 범위는 [QA 기록](docs/qa/2026-09-20-reliability.md) 참조.
- 이 변경은 로컬 작업 상태이며 커밋·푸시·배포와 원격 CI 실행은 하지 않음.

아래는 과거 시점의 이력이다. 이전 버전·미해결 목록·커밋 상태를 현재 상태로 해석하지 않는다.
