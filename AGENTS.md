...
| `POST /api/e` | 이용 통계 수집. **DB의 날짜·시각은 UTC+9(KST = JST) 기준**이며 `japanDay()`에서만 계산(서버 시간대 의존 금지)(`src/lib/analytics`). 원본 이벤트 없이 `agg_daily` 일별 카운트만 UPSERT, 세션은 날짜별 솔트 해시(31일 후 cron 삭제). 좌표·IP·본문 미수집. `DATABASE_URL` 없으면 204로 무동작. 진입 경로는 QR URL의 `?src=`·`?hotel=`로 구분 |
